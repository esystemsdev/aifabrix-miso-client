...
): response is ClientTokenResponse & { config: DataClientConfigResponse } {
...
function parseStatusFromMessage(msg: string): number | undefined {
  const m =
    ...
    msg.match(/status code\s+(\d+)/i) ||
    msg.match(/status\s+(\d+)/i);
  return m ? parseInt(m[1], 10) : undefined;
}

/** Get status/isNetwork from axios error */
function getAxiosResult(axiosError: import("axios").AxiosError): {
  status?: number;
...
function extractHttpStatus(tokenError: unknown): {
  status?: number;
  isNetworkError: boolean;
} {
  const msg =
...
function isNetworkOrTimeoutError(
  errorMessage: string,
  isAxiosNetworkError: boolean,
): { isNetwork: boolean; isTimeout: boolean } {
  const isNetwork =
    isAxiosNetworkError ||
    /ECONNREFUSED|ENOTFOUND|ETIMEDOUT|connect|Network Error|network error|getaddrinfo/i.test(
      errorMessage,
    );
  const isTimeout =
    errorMessage.includes("Request timeout: Failed to get environment token") ||
...
function handleTokenError(
  ...
  if (errorMessage.includes("Origin validation failed")) {
    sendErrorResponse(res, createErrorResponse(errorMessage, 403, req));
    return;
  }
  ...
  if (status) {
    const msg =
      status === 401
        ? "Failed to get environment token: Invalid client credentials or application not found"
        : status === 403
          ? "Failed to get environment token: Access denied"
          : errorMessage;
    sendErrorResponse(res, createErrorResponse(msg, status, req));
    return;
  }

  const { isNetwork, isTimeout } = isNetworkOrTimeoutError(
    errorMessage,
    isNetworkError,
  );
  if (isNetwork || isTimeout) {
    ...
    try {
      controllerUrl = resolveControllerUrl(config);
    } catch {
      controllerUrl =
        config.controllerPrivateUrl || config.controllerUrl || "NOT CONFIGURED";
    }
    const detail = isNetwork
      ? `Controller is unreachable. URL: ${controllerUrl}.`
      : `Controller request timed out. URL: ${controllerUrl}.`;
    sendErrorResponse(
      res,
      createErrorResponse(
        `Failed to get environment token: ${detail}`,
        504,
        req,
      ),
    );
...
function sendHandlerError(error: Error, req: Request, res: Response): void {
  ...
  if (/timeout/i.test(msg)) {
    sendErrorResponse(
      res,
      createErrorResponse(
        "Failed to get environment token: Request timed out",
        504,
        req,
      ),
    );
...
async function runClientTokenHandler(
  ...
  if (!misoClient.isInitialized()) {
    if (!isSent())
      sendErrorResponse(
        res,
        createErrorResponse("MisoClient is not initialized", 503, req),
      );
    return;
  }
  ...
  if (!token) {
    if (!isSent())
      sendErrorResponse(
        res,
        createErrorResponse(
          "Request timeout: Failed to get environment token",
          504,
          req,
        ),
      );
  ...
  if (opts.includeConfig) {
    ...
    if (!configResponse) {
      if (!isSent())
        sendErrorResponse(
          res,
          createErrorResponse("Controller URL not configured", 500, req),
        );
      return;
    }
