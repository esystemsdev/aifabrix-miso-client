...
// Serve index.html for root path with environment variables injected
app.get("/", (req, res) => {
  console.log(`[${new Date().toISOString()}] GET / - Request received`);
  try {
    const indexPath = join(process.cwd(), "public", "index.html");
    console.log(`[${new Date().toISOString()}] Checking index path: ${indexPath}`);
    if (existsSync(indexPath)) {
      console.log(`[${new Date().toISOString()}] Reading HTML file...`);
      let html = readFileSync(indexPath, "utf-8");
      console.log(`[${new Date().toISOString()}] HTML read successfully, length: ${html.length}`);
      
      // Get values from environment config
      const baseUrl = `http://localhost:${envConfig.port}`;
      const controllerUrl = (envConfig.misoControllerUrl || "").replace(/"/g, "&quot;");
      const clientId = (envConfig.misoClientId || "").replace(/"/g, "&quot;");
      console.log(`[${new Date().toISOString()}] Config values: baseUrl=${baseUrl}, controllerUrl=${controllerUrl}, clientId=${clientId}`);
      
      // Inject values into HTML form fields from .env
      ...
      const safeControllerUrl = escapeHtml(controllerUrl);
      const safeClientId = escapeHtml(clientId);
      
      console.log(`[${new Date().toISOString()}] Replacing values in HTML...`);
      // Replace baseUrl value - match id="baseUrl" anywhere in the tag, then find value="..."
      html = html.replace(
        /(<input[^>]*id="baseUrl"[^>]*value=")[^"]*(")/g,
        `$1${safeBaseUrl}$2`
      );
      html = html.replace(
        /(<input[^>]*id="controllerUrl"[^>]*value=")[^"]*(")/g,
        `$1${safeControllerUrl}$2`
      );
      html = html.replace(
        /(<input[^>]*id="clientId"[^>]*value=")[^"]*(")/g,
        `$1${safeClientId}$2`
      );
      
      console.log(`[${new Date().toISOString()}] Sending HTML response...`);
      res.setHeader("Content-Type", "text/html");
      res.send(html);
      console.log(`[${new Date().toISOString()}] HTML response sent successfully`);
    } else {
      console.log(`[${new Date().toISOString()}] Index file not found at ${indexPath}`);
      res.status(404).send("<h1>404</h1><p>Index page not found</p>");
    }
  } catch (error) {
    console.error(`[${new Date().toISOString()}] Error in root route:`, error);
