...
/**
 * Initialize DataClient with zero-config setup (recommended)
 * Automatically fetches configuration from server endpoint
 */
async function initializeDataClientZeroConfig() {
    try {
        showStatus('info', 'Initializing DataClient with zero-config setup...');
        updateConfigStatus('info', 'Fetching configuration from server...');

        // Check if autoInitializeDataClient is available (from real package)
        // For demo, we'll simulate it by fetching config and initializing
        const response = await fetch('/api/v1/auth/client-token', {
            method: 'POST',
            headers: {
        ...
        if (!data.config) {
            throw new Error('Server endpoint must use createClientTokenEndpoint() helper to provide config');
        }
        ...
        // Initialize DataClient with server-provided config
        dataClient = new window.DataClient({
        ...
        showStatus('success', 'DataClient initialized successfully with zero-config!');
        updateConfigStatus('success', 'DataClient is ready (zero-config)');
        updateCodeExamples();
    } catch (error) {
...
/**
 * Initialize DataClient with manual configuration
