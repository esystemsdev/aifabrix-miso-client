  ...
  return {
          ...
          for (const ext of extensions) {
            ...
            try {
              if (fs.existsSync(fullPath)) {
                ...
                if (stats.isFile()) {
                  console.log(
                    `[node-polyfills] Resolving ${id} (from services/logger) -> ${fullPath}`
                  );
                  return fullPath;
                }
              }
          ...
          for (const ext of extensions) {
            ...
            try {
              if (fs.existsSync(fullPath)) {
                ...
                if (stats.isFile()) {
                  console.log(
                    `[node-polyfills] Resolving ${id} (importer: ${importer || 'none'}) -> ${fullPath}`
                  );
                  return fullPath;
                }
              }
    ...
    transform(code, id) {
      ...
      if (id.includes('node_modules/ioredis') && code.includes('@ioredis/commands')) {
        ...
        code = code.replace(/from\s+['"]@ioredis\/commands['"]/g, `from '${stubRelativePath}'`);
        code = code.replace(/import\s+.*\s+from\s+['"]@ioredis\/commands['"]/g, (match) =>
          match.replace('@ioredis/commands', stubRelativePath)
        );

        if (code !== originalCode) {
          console.log(
            `[node-polyfills] ✅ Transformed @ioredis/commands imports in ${id.split('/').slice(-3).join('/')}`
          );
          return { code, map: null };
        } else {
          console.log(
            `[node-polyfills] ⚠️  No replacements made in ${id.split('/').slice(-3).join('/')}`
          );
        }
      }
      return null;
      ...
      plugins: [
        ...
        // This runs during the build phase and should catch imports from node_modules
        {
          ...
          resolveId(id, importer) {
            ...
            if (
              cleanId === 'dotenv' ||
              cleanId === 'dotenv/config' ||
              cleanId.startsWith('dotenv/')
            ) {
            ...
            if (cleanId.includes('services/logger')) {
              ...
              if (cleanId.startsWith('./services/logger') || cleanId === './services/logger') {
                ...
                cleanId.startsWith('../services/logger') ||
                cleanId === '../services/logger'
              ) {
                const loggerPath = path.join(rootDist, 'services', 'logger', 'index.js');
                if (fs.existsSync(loggerPath)) {
                  console.log(`[rollup-plugin] resolveId: ${id} -> ${loggerPath}`);
            ...
            if (
              cleanImporter &&
              cleanImporter.includes('services/logger') &&
              ...
              if (cleanImporter.includes('services/logger')) {
                ...
                for (const ext of extensions) {
                  ...
                  if (fs.existsSync(fullPath)) {
                    console.log(
                      `[rollup-plugin] resolveId: ${id} (from services/logger) -> ${fullPath}`
                    );
                    return fullPath;
                  }
                }
            ...
            if (
              cleanImporter &&
              cleanImporter.includes('express') &&
              (cleanId.startsWith('./') || cleanId.startsWith('../'))
            ) {
              const rootDist = path.resolve(__dirname, '../../dist');
            ...
            if (cleanId.startsWith('./') || cleanId.startsWith('../')) {
              ...
              if (
                ...
                looksLikeSDKImport ||
                !importer ||
                cleanImporter === cleanId ||
                (cleanImporter && cleanImporter.includes('?commonjs-external'))
              ) {
                let importerDir: string;
                ...
                if (
                  ...
                  for (const rootWorkspaceDist of possibleRootDists) {
                    if (fs.existsSync(rootWorkspaceDist)) {
                      ...
                      console.log(
                        `[rollup-plugin] Direct resolution from root dist: ${rootWorkspaceDist}, cleanId: ${cleanId}, importerDir: ${importerDir}`
                      );
                      break;
                    }
                  }
                ...
                if (
                  ...
                  cleanImporter !== cleanId &&
                  isFromSDK &&
                  !cleanImporter.includes('?commonjs-external')
                ) {
                  ...
                  if (fs.existsSync(pnpmPath)) {
                    try {
                      ...
                      const misoClientDir = pnpmDirs.find((dir: string) =>
                        dir.startsWith('@aifabrix+miso-client')
                      );
                      if (misoClientDir) {
                        const distPath = path.join(
                          pnpmPath,
                          misoClientDir,
                          'node_modules/@aifabrix/miso-client/dist'
                        );
                        const srcPath = path.join(
                          pnpmPath,
                          misoClientDir,
                          'node_modules/@aifabrix/miso-client/src'
                        );
                  ...
                  if (packageRoot) {
                    ...
                    console.log(
                      `[rollup-plugin] Found packageRoot: ${packageRoot}, cleanId: ${cleanId}, importerDir: ${importerDir}, will resolve to: ${path.resolve(importerDir, cleanId)}`
                    );
                  } else {
                    ...
                    const possibleDirs = [
                      path.resolve(
                        __dirname,
                        '../../node_modules/@aifabrix/miso-client/dist/utils'
                      ),
                      path.resolve(__dirname, '../../node_modules/@aifabrix/miso-client/src/utils'),
                ...
                if (looksLikeSDKImport) {
                  ...
                  if (fs.existsSync(rootWorkspaceDist)) {
                    ...
                    for (const ext of extensions) {
                      ...
                      if (fs.existsSync(fullPath)) {
                        console.log(
                          `[rollup-plugin] resolveId: ${id} (root workspace dist first) -> ${fullPath}`
                        );
                        return fullPath;
                      }
                    }
                ...
                if (resolvedPath) {
                  for (const ext of extensions) {
                    ...
                    if (fs.existsSync(fullPath)) {
                      console.log(
                        `[rollup-plugin] resolveId: ${id} (importer: ${importer || 'none'}) -> ${fullPath}`
                      );
                      return fullPath;
                    }
                  }
                ...
                if (looksLikeSDKImport && (!resolvedPath || !fs.existsSync(resolvedPath + '.js'))) {
                  ...
                  if (fs.existsSync(rootWorkspaceDist)) {
                    ...
                    for (const ext of extensions) {
                      ...
                      if (fs.existsSync(fullPath)) {
                        console.log(
                          `[rollup-plugin] resolveId: ${id} (root workspace dist fallback) -> ${fullPath}`
                        );
                        return fullPath;
                      }
                    }
                ...
                if (looksLikeSDKImport || isFromSDK) {
                  ...
                  for (const rootWorkspaceDist of possibleRootDists) {
                    if (fs.existsSync(rootWorkspaceDist)) {
                      ...
                      for (const ext of extensions) {
                        ...
                        if (fs.existsSync(fullPath)) {
                          console.log(
                            `[rollup-plugin] resolveId: ${id} (fallback to root dist: ${rootWorkspaceDist}) -> ${fullPath}`
                          );
                          return fullPath;
                        }
                      }
                  ...
                  console.log(
                    `[rollup-plugin] WARNING: Could not resolve ${id} from importer ${importer || 'none'}, tried ${resolvedPath || 'no path'}`
                  );
                  ...
                  if (
                    ...
                    const lastResortPath = path.resolve(
                      __dirname,
                      '../../../../dist/utils',
                      cleanId.substring(2) + '.js'
                    );
                    if (fs.existsSync(lastResortPath)) {
                      console.log(
                        `[rollup-plugin] Last resort resolution: ${id} -> ${lastResortPath}`
                      );
                  ...
                  if (looksLikeSDKImport) {
                    const absolutePath = path.resolve(
                      __dirname,
                      '../../../../dist/utils/response-validator.js'
                    );
                    if (fs.existsSync(absolutePath)) {
                      console.log(
                        `[rollup-plugin] Absolute path resolution: ${id} -> ${absolutePath}`
                      );
                      return absolutePath;
                    }
                  }
