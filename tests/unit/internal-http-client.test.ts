...
describe("InternalHttpClient", () => {
      ...
      (testMockAxiosInstance.get as jest.Mock).mockImplementation(
        async (url, config) => {
          ...
          if (requestInterceptor) {
            ...
            const interceptorConfig: InternalAxiosRequestConfig = (config || {
              headers: {},
            ...
            expect(updatedConfig.headers?.["x-client-token"]).toBe(
              "test-token-123",
            );
  ...
  describe("isAxiosError", () => {
    ...
    it("should return false for non-AxiosError objects", () => {
      ...
      return expect(httpClient.get("/test")).rejects.not.toThrow(
        MisoClientError,
      );
    });
  ...
  describe("requestWithAuthStrategy", () => {
    it("should make GET request with auth strategy", async () => {
      ...
      const result = await httpClient.requestWithAuthStrategy(
        "GET",
        "/test",
        authStrategy,
      );
    ...
    it("should throw error for unsupported method", async () => {
      ...
      await expect(
        httpClient.requestWithAuthStrategy(
          "PATCH" as any,
          "/test",
          authStrategy,
        ),
