...
jest.mock('../../src/utils/sensitive-fields.config.json', () => ({
  __esModule: true,
  default: {
    version: '1.0.0',
...
describe('sensitive-fields.loader', () => {
  beforeEach(() => {
    ...
    (path.isAbsolute as jest.Mock).mockImplementation((p: string) => realPath.isAbsolute(p));
    (path.resolve as jest.Mock).mockImplementation((...paths: string[]) => realPath.resolve(...paths));
    (path.join as jest.Mock).mockImplementation((...paths: string[]) => realPath.join(...paths));
  });
  ...
  describe('loadSensitiveFieldsConfig', () => {
    ...
    it('should try filesystem path when module import fails', () => {
      const configPath = path.join(__dirname, '../../src/utils/sensitive-fields.config.json');
      const configContent = JSON.stringify({
        version: '1.0.0',
        description: 'Test config',
        categories: {
