...
describe("LoggerService Getter Methods", () => {
  ...
  describe("getWithToken", () => {
    it("should return LogEntry with token context extracted", () => {
      mockedJwt.decode.mockReturnValue({
        sub: "user-789",
      ...
      const logEntry = loggerService.getWithToken(
        "jwt-token-here",
        "Token validated",
        "audit",
        { action: "validate" },
      ...
      expect(logEntry).toBeDefined();
      expect(logEntry.message).toBe("Token validated");
      expect(logEntry.level).toBe("audit");
      expect(logEntry.userId).toBe("user-789");
      expect(logEntry.sessionId).toBe("session-999");
      expect(logEntry.applicationId).toBe("app-123");
      expect(logEntry.correlationId).toBeDefined();
      expect(logEntry.context).toEqual({ action: "validate" });
      expect(logEntry.timestamp).toBeDefined();
    });

    it("should extract userId from various token fields", () => {
      mockedJwt.decode.mockReturnValue({
        userId: "user-from-userId",
      });

      const logEntry = loggerService.getWithToken("token", "Test");

      expect(logEntry.userId).toBe("user-from-userId");
    });
    ...
    it("should extract userId from user_id field", () => {
      ...
      expect(logEntry.userId).toBe("user-from-user_id");
    });
    ...
    it("should handle missing token fields", () => {
      ...
      expect(logEntry.userId).toBeUndefined();
      expect(logEntry.sessionId).toBeUndefined();
      expect(logEntry.applicationId).toBe("");
    });

    it("should handle null decoded token", () => {
      mockedJwt.decode.mockReturnValue(null);

      const logEntry = loggerService.getWithToken("invalid-token", "Test");
    ...
    it("should mask sensitive data in context by default", () => {
      ...
      const logEntry = loggerService.getWithToken("token", "Test", "info", {
        password: "secret",
      });
    ...
    it("should generate correlation ID automatically", () => {
      ...
      const logEntry1 = loggerService.getWithToken("token1", "Message 1");
      const logEntry2 = loggerService.getWithToken("token2", "Message 2");
