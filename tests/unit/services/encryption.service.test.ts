...
describe("EncryptionService", () => {
  ...
  describe("encrypt", () => {
    ...
    it("should throw EncryptionError for empty parameter name", async () => {
      await expect(encryptionService.encrypt("secret", "")).rejects.toThrow(
        EncryptionError,
      );
    });
    ...
    it("should propagate API errors", async () => {
      ...
      (mockApiClient.encryption.encrypt as jest.Mock).mockRejectedValue(
        apiError,
      );
  ...
  describe("decrypt", () => {
    ...
    it("should throw EncryptionError for invalid parameter name", async () => {
      await expect(encryptionService.decrypt("kv://param", "")).rejects.toThrow(
        EncryptionError,
      );
    });
    ...
    it("should propagate API errors", async () => {
      ...
      (mockApiClient.encryption.decrypt as jest.Mock).mockRejectedValue(
        apiError,
      );
  ...
  describe("encryption cache", () => {
    ...
    it("encrypt cache hit: second encrypt returns cached result, API not called again", async () => {
      const mockCache = createMockCache();
      const cachedResult: EncryptResult = {
        value: "kv://cached",
        storage: "keyvault",
      };
    ...
    it("controller error on encrypt: cache.set is not called", async () => {
      ...
      await expect(svc.encrypt("secret", "my-param")).rejects.toThrow(
        "API error",
      );
      expect(mockCache.set).not.toHaveBeenCalled();
    });
    ...
    it("controller error on decrypt: cache.set is not called", async () => {
      ...
      await expect(svc.decrypt("kv://x", "x")).rejects.toThrow(
        "Decrypt failed",
      );
      expect(mockCache.set).not.toHaveBeenCalled();
    });
    ...
    it("validation error: no cache get or set", async () => {
      ...
      await expect(svc.encrypt("secret", "invalid name")).rejects.toThrow(
        EncryptionError,
      );
