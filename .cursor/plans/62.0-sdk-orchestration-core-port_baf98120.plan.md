...
- Downstream-gated closure and legacy deprecation/removal closure: pending.
...
- `setupActivityDrivenRefreshListener` remains as a frozen wrapper and is explicitly marked `@deprecated`.
...
- Legacy compatibility entrypoint `setupActivityDrivenRefreshListener` is now explicitly marked `@deprecated` and frozen as wrapper-only path; parity path uses `setupSessionRecoveryOrchestrationListener`.
