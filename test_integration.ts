...
class TestRunner {
  ...
  async runTest(
    ...
    if (skip) {
      ...
      console.log(
        `  ${colors.yellow("⊘")} ${name} ${colors.yellow("(skipped)")}`,
      );
      return;
    }
    ...
    try {
      ...
      if (shouldSkip) {
        console.log(
          `  ${colors.yellow("⊘")} ${name} ${colors.yellow("(skipped)")} ${colors.blue(durationStr)}`,
        );
...
async function runIntegrationTests(): Promise<void> {
  ...
  if (!controllerAvailable) {
    console.log(
      `${colors.yellow("⚠")} Controller authentication failed at ${config.controllerUrl}`,
    );
    console.log(
      `${colors.yellow("⚠")} Tests will run but may fail with authentication errors\n`,
    );
  }
    ...
    async () => {
      try {
        ...
        if (
          ...
          throw new Error(
            "SKIP: Controller credentials invalid - skipping client token tests",
          );
        }
        throw error;
      }
  ...
  await runner.runTest(
    "login returns response with loginUrl",
    async () => {
      try {
        ...
        if (
          !response.data.loginUrl.startsWith("http://") &&
          !response.data.loginUrl.startsWith("https://")
        ) {
          throw new Error(
            "login URL should be a valid URL (starts with http:// or https://)",
          );
        }
    ...
    async () => {
      try {
        ...
        if (
          ...
          throw new Error(
            "SKIP: Logout requires valid token (test uses invalid token)",
          );
        }
        throw error;
      }
  ...
  await runner.runTest("hasAnyPermission", async () => {
    suppressErrors();
    try {
      const hasAny = await client.hasAnyPermission(apiKey, [
        "read:users",
        "write:users",
      ]);
      if (typeof hasAny !== "boolean") {
  ...
  await runner.runTest("hasAllPermissions", async () => {
    suppressErrors();
    try {
      const hasAll = await client.hasAllPermissions(apiKey, [
        "read:users",
        "write:users",
      ]);
      if (typeof hasAll !== "boolean") {
  ...
  await runner.runTest("info logging", async () => {
    await client.log.info("Integration test info message", {
      test: "integration",
    });
  });
  ...
  await runner.runTest("audit logging", async () => {
    await client.log.audit("test_action", "test_resource", {
      test: "integration",
    });
  });

  await runner.runTest("debug logging", async () => {
    await client.log.debug("Integration test debug message", {
      test: "integration",
    });
  });
  ...
  await runner.runTest("Redis fallback to in-memory", async () => {
    ...
    const success = await client.cache.set(
      "test:key:4",
      { test: "memory" },
      60,
    );
