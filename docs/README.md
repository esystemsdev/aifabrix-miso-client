...
| **URL strategy**    | [configuration.md#full-urls-and-virtual-directories](configuration.md)               | Full roots, browser path-only `controllerPublicUrl`, optional `controllerBasePath` / DataClient `basePath`, `joinApiRoot` + `mergeRootUrlWithBasePath` + `coerceControllerUrlToAbsolute`; avoiding double-prefix |
| **DataClient**      | [dataclient.md](dataclient.md)                                                       | Browser client: zero-config (backend route + autoInitializeDataClient), main methods                                                                                                                             |
| **Browser UI auth** | [authentication.md#browser-ui-helpers-416](authentication.md#browser-ui-helpers-416) | Cookie session, split-port base URL, silent recovery (`createCookieSessionCallbacks`, `resolveBrowserApiBaseUrl`, etc.) — 4.16+                                                                                  |
