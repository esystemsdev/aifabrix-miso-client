...
- `params.redirect` - Required callback URL where Keycloak redirects after authentication
- `params.state` - Optional CSRF protection token (auto-generated by controller if omitted)

**Returns:** Promise resolving to `LoginResponse` with `loginUrl` and `state`

**Example:**

```typescript
// In browser environment - redirect to login URL
...
const response = await client.login({ redirect: 'https://myapp.com/callback' });
