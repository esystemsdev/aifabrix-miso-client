...
async function testLoggingWithJWT() {
  ...
  const testToken =
    ...
    "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJ0ZXN0LXVzZXItMTIzIiwic2Vzc2lvbklkIjoic2Vzc2lvbi00NTYiLCJhcHBsaWNhdGlvbklkIjoiYXBwLTc4OSIsImlhdCI6MTYwOTQ1NjgwMCwiZXhwIjo5OTk5OTk5OTk5fQ.test-signature";
...
async function testRedisConnection() {
  ...
  try {
    ...
    console.log(
      `   Redis connection status: ${isConnected ? "✅ Connected" : "❌ Not connected"}`,
    );
    ...
    if (isConnected) {
      ...
      if (setResult) {
        ...
        if (getResult === testValue) {
          ...
          console.log(
            `   ⚠️  Redis GET returned unexpected value: ${getResult}`,
          );
        }
...
async function testDirectAxiosCalls(): Promise<"skipped" | void> {
  ...
  try {
    ...
    console.log(
      "   ⚠️  Skipping direct axios tests (credentials may not be valid for this controller)\n",
    );
  ...
  try {
    ...
    if (response.status === 200 || response.status === 201) {
      console.log(
        `   ✅ Unified endpoint works (status: ${response.status})\n`,
      );
    } else {
      console.log(
        `   ❌ Unified endpoint failed: ${JSON.stringify(response.data)}\n`,
      );
    }
  } catch (error) {
    console.error(
  ...
  try {
    ...
    if (
      ...
      batchResponse.status === 207
    ) {
      console.log(
        `   ✅ Batch endpoint works (status: ${batchResponse.status})\n`,
      );
    } else {
      console.log(
        `   ❌ Batch endpoint failed: ${JSON.stringify(batchResponse.data)}\n`,
      );
    }
  } catch (error) {
    console.error(
      "   ❌ Batch endpoint error:",
      error instanceof Error ? error.message : String(error),
    );
  }
}
...
async function testXClientTokenAuth(): Promise<"skipped" | void> {
  ...
  try {
    ...
    console.log(
      "   ⚠️  Skipping x-client-token auth tests (credentials may not be valid for this controller)\n",
    );
    return "skipped"; // Skip this test instead of failing
  }
  ...
  try {
    ...
    if (response.status === 200 || response.status === 201) {
      ...
      console.log(
        `   ❌ x-client-token auth failed: ${JSON.stringify(response.data)}\n`,
      );
    }
  } catch (error) {
    console.error(
      "   ❌ x-client-token auth error:",
      error instanceof Error ? error.message : String(error),
    );
  }
...
async function testApplicationIdExtraction(): Promise<"skipped" | void> {
  ...
  try {
    ...
    if (parts.length === 3) {
      ...
      console.log(
        `   Expected applicationId from token: ${expectedApplicationId}`,
      );
    }
  } catch (e) {
  ...
  try {
    ...
    console.log(
      "   ⚠️  Skipping applicationId extraction test (credentials may not be valid for this controller)\n",
    );
    return "skipped"; // Skip this test instead of failing
  }
  ...
  try {
    ...
    if (response.status === 200 || response.status === 201) {
      ...
      console.log(
        `   ✅ Should be associated with applicationId: ${expectedApplicationId}\n`,
      );
    } else {
...
async function runAllTests() {
  ...
  if (failed > 0) {
    ...
    if (skipped > 0) {
      console.log(
        `   (${skipped} test(s) skipped - may need valid controller credentials)`,
      );
    }
    return 0;
  } else {
